        ...
        <div class="bg-white relative border-solid border border-gray-300 rounded-lg sm:rounded-lg overflow-hidden">
            <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                ...
                <div
                    ...
                    <a href="{{ url('rubros/create') }}"
                        ...
                        <button type="button">Nuevo rubro</button>
                    </a>
                </div>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left text-gray-500 ">
                        ...
                        @forelse ($rubros as $rubro)
                            <tr class="border-b">
                                <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap">
                                    #{{ $rubro->id }}</th>
                                <td class="px-4 py-3">{{ $rubro->rubro }}</td>
                                <td class="px-4 py-3">{{ $rubro->created_at }}</td>
                                <td class="px-4 py-3 flex items-center justify-center align-items-center">
                                    <a href="{{ url('rubros/' . $rubro->id . '/edit') }}"
                                    ...
                                    <form action="{{ route('rubros.destroy', $rubro->id) }}" method="post"
                                        onclick="Eliminar(this, '{{ $rubro->rubro }}')">
                                        @csrf
                                        @method('delete')
                    ...
                    <p class="text-center text-slate-950 my-5">Todavia no existen rubros registrados</p>
                    @endforelse
                </table>
            </div>
            @if (count($rubros))
...
@include('rubros.modal')


<script>
    function Eliminar(e, rubro) {
        Swal.fire({
            title: "Estas seguro?",
            text: `Estas a punto de eliminar al rubro "${rubro}"`,
            icon: "warning",
    ...
    function data(registro) {
        return `
                <tr class="border-b">
                    <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap">
                        #${registro.id}</th>
                    <td class="px-4 py-3">${registro.rubro}</td>
                    <td class="px-4 py-3">${registro.created_at }</td>
                    <td class="px-4 py-3 flex items-center justify-center align-items-center">
                        @can('rubros.update')
                        <a href="{{ url('rubros') }}/${registro.id}/edit"
                            class="hover:cursor-pointer hover:bg-gray-100">
                            <svg class="w-6 h-6 text-gray-800" aria-hidden="true"
                                xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                fill="none" viewBox="0 0 24 24">
                        ...
                        @endcan
                        @can('rubros.destroy')
                        <form action="{{ url('rubros') }}/${registro.id}" method="post"
                            onclick="Eliminar(this, '${registro.rubro}')">
                            @csrf
                            @method('delete')
                            <button type="button" class="hover:cursor-pointer hover:bg-gray-100 mt-1">
                                <svg class="w-6 h-6 text-gray-800" aria-hidden="true"

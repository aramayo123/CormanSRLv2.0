                            ...
                            <tr class="border-b">
                                ...
                                <td class="px-4 py-3 flex items-center justify-center align-items-center">
                                    ...
                                    <form action="{{ route('sucursales.destroy', $sucursal->id) }}" method="post"
                                        onclick="Eliminar(this, '{{ $sucursal->sucursal }}')">
                                        @csrf
                                        @method('delete')
...
<script>
    ...
    const limit_for_pag = 7;
    var text_max_pag = document.querySelector('#max_pag');
    var registros = @json($sucursales);
    ...
    function data(registro) {
        return `
            <tr class="border-b">
                ...
                <td class="px-4 py-3">${registro.numero }</td>
                <td class="px-4 py-3">${registro.sucursal }</td>
                <td class="px-4 py-3">${registro.direccion }</td>
                <td class="px-4 py-3">${registro.created_at }</td>
                <td class="px-4 py-3 flex items-center justify-center align-items-center">
                    @can('sucursales.update')
                     <a href="{{ url('sucursales') }}/${registro.id}/edit"
                        class="hover:cursor-pointer hover:bg-gray-100">
                        <svg class="w-6 h-6 text-gray-800" aria-hidden="true"
                            xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                            fill="none" viewBox="0 0 24 24">
                    ...
                    @endcan
                    @can('sucursales.destroy')
                    <form action="{{ url('sucursales')}}/${registro.id}" method="post" onclick="Eliminar(this, '${registro.sucursal}')">
                        @csrf
                        @method('delete')
                        <button type="button" class="hover:cursor-pointer hover:bg-gray-100 mt-1">
                            <svg class="w-6 h-6 text-gray-800" aria-hidden="true"
    ...
    function KeyUpDown(event) {
        ...
        if (buscador.value) {
            ...
            for (var i = 0; i < filas.length; i++) {
                var textoFila_0 = filas[i].children[1].innerText.toLowerCase() || filas[i].children[1].textContent
                    .toLowerCase();
                var textoFila_1 = filas[i].children[2].innerText.toLowerCase() || filas[i].children[2].textContent
                    .toLowerCase();
                var textoFila_2 = filas[i].children[3].innerText.toLowerCase() || filas[i].children[3].textContent
                    .toLowerCase();
                var textoFila_3 = filas[i].children[4].innerText.toLowerCase() || filas[i].children[4].textContent
                    .toLowerCase();

                if (textoFila_0.includes(filtro) || textoFila_1.includes(filtro) || textoFila_2.includes(filtro) ||

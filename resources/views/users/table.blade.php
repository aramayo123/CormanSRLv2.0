@php
    ...
    foreach ($users as $user) {
        $user->role = $user->hasRole('Corman') ? 'Corman' : ($user->hasRole('Facilitie') ? 'Facilitie' : 'Operario');
        array_push($users_modify, $user);
    }
@endphp
...
<script>
    ...
    var registros = @json($users_modify);
    ...
    function data(registro){
            return `
                ...
                <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap">
                    #${registro.id}</th>
                <td class="px-4 py-3">${registro.name}</td>
                <td class="px-4 py-3">${registro.username}</td>
                <td class="px-4 py-3">${registro.email}</td>
                <td class="px-4 py-3">${registro.role}</td>
                <td class="px-4 py-3">${registro.created_at }</td>
                <td class="px-4 py-3 flex items-center justify-center align-items-center">
                    @can('users.update')
                        <a href="{{ url('users') }}/${registro.id}/edit"
                            class="hover:cursor-pointer hover:bg-gray-100">
                            <svg class="w-6 h-6 text-gray-800" aria-hidden="true"
                                xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                fill="none" viewBox="0 0 24 24">
                    ...
                    @endcan
                    @can('users.destroy')
                        <form action="{{ url('users') }}/${registro.id}" method="post"
                            onclick="Eliminar(this, '${registro.name}')">
                            @csrf
                            @method('delete')
                            <button type="button" class="hover:cursor-pointer hover:bg-gray-100 mt-1">
                                <svg class="w-6 h-6 text-gray-800" aria-hidden="true"
    ...
    function KeyUpDown(event){
        ...
        if (buscador.value) {
            ...
            for (var i = 0; i < filas.length; i++) {
                var textoFila_1 = filas[i].children[1].innerText.toLowerCase() || filas[i].children[1].textContent.toLowerCase(); 
                var textoFila_2 = filas[i].children[2].innerText.toLowerCase() || filas[i].children[2].textContent.toLowerCase();
                var textoFila_3 = filas[i].children[3].innerText.toLowerCase() || filas[i].children[3].textContent.toLowerCase();
                
                if (textoFila_1.includes(filtro) || textoFila_2.includes(filtro) || textoFila_3.includes(filtro))
